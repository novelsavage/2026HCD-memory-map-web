...
- `?nocity` で無効化
- `?plateau` で建物だけ `public/plateau-surroundings.json` へ差し替える。
  PLATEAU 柏市LOD1の建物外形と実測高さを利用し、道路・鉄道・駅はOSMを併用。
  変換は `scripts/convert-plateau.mjs`。配信JSONは緯度経度を小数第6位へ丸める
- **ライセンス**: ODbL。「© OpenStreetMap contributors」表示が必須
  （画面右下 + README に記載済み）。PLATEAUモードでは
  「国土交通省 Project PLATEAU 3D都市モデル（柏市・2020年度）」も表示
...
| `?plateau` | 周辺建物をPLATEAU柏市LOD1へ差し替え |
| `?nocity` | 周辺市街地を非表示 |
...
- [x] PLATEAU 柏市LOD1の試験表示（`?plateau`）。OSMとの比較後に本線採用を判断
