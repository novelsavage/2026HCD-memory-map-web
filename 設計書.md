...
├─ Cylinder(pole)  … ジャンル色、高さ 5–11 でランダム
...
- 根元 Y = 地面 + `MARKER.groundOffsetY`（Unity `MemoryGeoProjector.worldHeight=10` を Web 座標系へ換算）
- ポール: Unity `Pin.prefab` 相当（高さ 40 unit × 1/0.29 ≈ 138）。根元から地面方向へ伸びる
- テキスト: 32×8 unit × 1/0.29（Plane 約 110×28）、根元から `labelOffsetY` 上
...
- `memory_text` を DotGothic16 で Canvas 描画 → `CanvasTexture`
