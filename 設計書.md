...
キャンパスは丘陵地形でモデル内の高低差が約 50m あるため、周辺台地
（campus bbox min 基準）に対してモデルの縁の大部分が浮き、「空中に浮いた島」に
見える問題があった（2026-07-06 メンバー指摘）。対策として `buildCampusSkirt()` が
起動時にメッシュの**境界エッジ**（1 つの三角形にしか属さないエッジ。頂点を 0.25m
グリッドに量子化して Draco 誤差とタイル継ぎ目を吸収しつつ検出）から台地までの
垂直壁を生成し、暗い丘の斜面として虚空を埋める。約 7.2 万エッジ・生成 0.6 秒。
`?noskirt` で無効化。
