...
広域マップ化の構想は [docs/wide-area-map-plan.md](docs/wide-area-map-plan.md) を参照。
...
└── src/
    ...
    ├── config.ts         # ★調整値の集約地。キャリブレーション/色/カード寸法
    ├── scene.ts          # renderer/camera/controls/ライト/星空/グリッド/bloom
    ├── campus.ts         # GLB 読込・表示モード切替・地面スナップ raycast
    ├── geo.ts            # 緯度経度 → ワールド座標投影
    ├── data.ts           # Supabase 取得 + デモ JSON フォールバック
    ├── cards.ts          # 思い出カード群の生成・アニメ・フィルター・ピッキング
    ├── cardTexture.ts    # card_url 読込 + Canvas 代替カード生成
...
main()
 ...
 │  ])
 ├─ cards.spawn(memories, snapToGround)  … 投影 + 地面レイキャスト + メッシュ生成
...
| カラム | 型 | 本サイトでの扱い |
|---|---|---|
| `id` | text PK（例 `HCD-20260613-141847-A9UJ`） | カード識別・フォーカス対象特定 |
| `status` | `published` / `hidden` | **RLS により anon には published しか返らない**（クエリでも明示フィルタ） |
| `memory_text` | text | 詳細パネル・代替カード本文 |
| `nickname` | text | 詳細パネル・代替カード署名 |
| `genre` | text スラッグ（enum ではない） | 色分け・フィルター。未知の値は `GENRE_FALLBACK_COLOR` |
| `era` | text（例 `1980年代`。enum ではない） | フィルター。null は「全年代選択時のみ表示」 |
| `latitude` / `longitude` | double | §6 の投影に使用。null なら浮遊扱い |
| `card_url` | text（R2 公開 PNG） | カードテクスチャ。CORS 失敗時は Canvas 代替（§8） |
...
- `distanceMeters > maxDistanceFromOriginMeters(3000)` の座標は異常値として
  浮遊表示に落とす（Unity 版の安全弁と同じ発想）
...
4. Geo 系の変更はカード再配置が必要なので**リロードして確認**

> 現状 (2026-07-05) キャリブレーション未実施。モデル bbox は
> (262.6 × 15.0 × 242.1)、中心 (−1.4, 21.0, −2.8) で原点とほぼ一致しているが、
> yaw と縮尺の実測合わせはまだ。
...
## 8. 思い出カード（cards.ts / cardTexture.ts）

### 構造（1 件あたり）

```
Group (root)  ← 位置・出現スケール
├─ [大学内のみ] Cylinder(pole)  … ジャンル色、高さ 5–11 でランダム
├─ [大学内のみ] Sphere(pin)     … ジャンル色、接地点
└─ Plane 9×9 "card"             … テクスチャ。毎フレーム camera.quaternion をコピー
    └─ [浮遊のみ] Plane 9.72×9.72 … ジャンル色の縁取り
```

- **ビルボードは card メッシュのみ回転**させ、root/pole は傾けない
  （Unity 版 `MemoryMarker.LateUpdate` と同じ手法）
- 配置判定: `reitaku_dummy && 座標あり && 距離3000m以内` → 地上ピン。
  それ以外 → キャンパス上空リング（半径 240–340、高さ 45–100）に均等角+ジッターで
  浮遊配置し、sin 波で上下にゆらす
- 地面高さは `snapToGround()`（y=500 から下向き raycast）で取得。
  モデル外に落ちた場合は y=0
- 出現アニメ: 1 件ごとに 0.07s ずらした easeOutBack スケールイン。
  フィルター切替も同じ機構で縮小消滅（`visible` フラグ + `appear` 0..1 補間）
- ピッキング: pointerup 時に移動量 6px / 押下 400ms 以内なら Raycaster でカード判定
  （OrbitControls のドラッグと区別するため）

### カードテクスチャの決定順

1. `card_url` あり → `TextureLoader`（crossOrigin=anonymous）で R2 の PNG
2. 失敗（CORS / 404）or `card_url` null → **Canvas で代替カード生成**:
   白台紙 512² + ジャンル色帯 + 年代 + 本文（文字単位折返し）+ ニックネーム。
   フォントは DotGothic16

> R2 バケットに CORS 設定が無い場合は全カードが代替表示になる。
> 実画像を出すには R2 側で `AllowedOrigins` に配信ドメインを追加する。
...
- 範囲: OCR WebApp `capture-form.tsx` の `MAP_BOUNDS` と同一
  （35.8243–35.8465 / 139.9397–139.9655、約 2.3km × 2.5km。南柏駅を含む。
  新柏駅は東側の範囲外）
...
- キャンパスモデルの XZ バウンディングボックス内の建物は生成しない
  （Google Earth モデルと重複するため）
- 台地プレーンは `SURROUNDINGS.baseY (13.2)` = キャンパスモデルの底面高さに合わせる
- 建物屋上・台地面はカードの接地 raycast 対象に含まれる
- `?nocity` で無効化
- **ライセンス**: ODbL。「© OpenStreetMap contributors」表示が必須
  （画面右下 + README に記載済み）

### カード接地ルール（cards.ts、Phase 2 で変更）

旧: `reitaku_dummy=true` かつ原点 3km 以内 → 接地
新: **座標が `MAP_BOUNDS` 内なら接地**（reitaku_dummy に依存しない）。
範囲外・座標なしは従来どおり上空リングに浮遊。
...
- 詳細パネル: 右スライドイン。カードクリックでカメラも対象へ lerp フォーカス
...
- [ ] 座標キャリブレーション未実施（§6。実データ投入後に要調整）
- [ ] Supabase 接続情報が未設定（森さんの Supabase プロジェクトから取得）
- [ ] R2 の CORS 未確認（実カード PNG が WebGL テクスチャとして読めるか）
- [ ] カード数百件超でのパフォーマンス未検証
      （1000 件超なら InstancedMesh / テクスチャアトラス化を検討）
- [ ] モバイル実機での操作性・負荷未検証
